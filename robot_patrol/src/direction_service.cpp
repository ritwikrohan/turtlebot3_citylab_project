...
class DirectionService : public rclcpp::Node{
    ...
    private: 
        ...
        double MinRay(const std::vector<float>& range, int start_index) 
        {
            ...
            for (int i = start_index-25; i <= end_index; i++) {
        ...
        void direction_callback(const std::shared_ptr<robot_patrol::srv::GetDirection::Request> req, const std::shared_ptr<robot_patrol::srv::GetDirection::Response> res) {
            ...
            RCLCPP_DEBUG(this->get_logger(), "left: %f (max: %f)", total_dist_sec_left, max_dist_sec_left);
            RCLCPP_DEBUG(this->get_logger(), "right: %f (max: %f)", total_dist_sec_right, max_dist_sec_right);
            RCLCPP_DEBUG(this->get_logger(), "forward: %f (max: %f)", total_dist_sec_front, max_dist_sec_front);
